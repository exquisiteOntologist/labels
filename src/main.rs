...
fn my_basic_experiment() -> Result<(), Box<dyn Error>> {
    ...
    // let cleansed_words: Vec<&str> = words
    //     .into_iter()
    //     .map(|w| &w.replace(&['\'', '.'], ""))
    //     .collect();
    // let next_words: Vec<&str> = cleansed_words
    //     .as_ref()
    //     .into_iter()
    //     .map(|w| w.as_str())
    //     .collect();
    ...
    for word in words {
        ...
        // let mut clean_word: &str = "";
        // for c in word.chars() {
        //     if ex_chars.any(|ex| c == ex) == false {
        ...
        // let mut clean_word = &mut word.replace(&['\''], "");

        println!("chars {:?}", word.chars().as_str());

        let last_char = word.char_indices().nth_back(0).unwrap().1;
        let last_in_sentence = last_char == '.' || last_char == ',';

        // let partial: &str = &word[..word.len()];
        //
        let mut word_clean: &str = word;

        if last_in_sentence {
            word_clean = &word[..word.len() - 1]
        }
    ...
    // let mut word_tallies: Vec<Vec<&str>> = vec![vec![]];

    // words_wanted.group_by(|a,b| a == b);
    // words_wanted.into_iter().map
    // split where different to each other

    // count similars
    let mut word_tallies: Vec<(Vec<&str>, i32)> = vec![];
    let mut prior_word: Vec<&str> = Default::default();
    let mut tally: i32 = 0;
    for (i, word) in words_wanted.into_iter().enumerate() {
